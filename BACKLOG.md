...
- [ ] **⚠️ OWNER DECISION — the hit-count thresholds are not HAAG's.** Research (docs/THRESHOLD_PROVENANCE.md, with sources) found HAAG publishes NO hit-count threshold at all: its test square exists to count damaged units for the `D x U x R x A` extrapolation, and its damage test is qualitative (punctures/tears/mat fractures). The 8 / 7-10 / 8-10 numbers are CARRIER conventions. Our §2 `>5` for 3-tab is stricter than every carrier norm found, so the app calls replacement at counts adjusters reject — wrong in the expensive direction. Three options in the doc; recommendation is C (keep the trigger, state the count against BOTH bars using the already-written, currently-unused `CARRIER_IMPACT_NORM_NOTE`). Do not change a documented threshold without the owner — #69
...
- [ ] **Per-photo damage report + non-roof/collateral identification** — spec written in `docs/SPEC_photo_report_and_engine_bug.md`: tap an analyzed photo for a short report (hits, damage type, shingle/roof type, which side) and, when the frame is not a roof, identify what WAS captured and its damage. Note for whoever picks this up: the "No Functional Damage / Repair-Monitor / 6.5 affected squares" and "27 of 100 — Wear consistent with age" strings in the owner's screenshots do **not** exist anywhere in this repo at any commit (verified `git grep` + `git log -S` across all history) — those screenshots are of the REFERENCE app whose layout the owner liked, not RoofWise output. Do not go hunting for that label in our code — #65, #66
