...
- [ ] **Property-record follow-ups (#89)** — (a) the HAAG packet / Long Report / proposal covers should carry the job's cover photo (`coverPhotoUri`) — today only the screens do; (b) leads: look the house up on lead creation too so the Leads board fronts with it; (c) Settings: an APIllow usage row (`fetchApillowUsage`, free tier 50/month) so the roofer sees the quota; (d) `backup.ts` does not include `documentDirectory/covers/` (inspector-chosen photos) — add it; (e) on-device: confirm the camera/library copy path and that Zillow photos render through Expo Go — #89
