...
- [ ] **Owner decisions from Wave D** — `brittleness_observed` tick → FAIL (or BORDERLINE?); roof-level `mat_exposed` lands on damaged slopes and can move existing REPAIR jobs to PARTIAL_REPLACEMENT; evidence-less legacy hits read not-functional after any marker edit until re-analyzed; coordinate-only addresses cannot be saved as real — #84
