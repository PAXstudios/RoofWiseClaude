...
- [ ] **AUDIT P0 (2026-09-03) — Quick Inspection's placeholder job can never be corrected.** `quick-inspection.tsx` auto-creates "Quick inspection" / "Address pending" / architectural / good and the job screen has no customer/address/material editor, no store action. Add `updateDetails` + an editable Property card; on Quick Inspection Done, require name/address (prefill from GPS reverse-geocode) before the packet CTA — #80
- [ ] **AUDIT P0 — the proposal Share link is a dead URL** (`roofwise.app/p/<token>` renders from local AsyncStorage; nothing hosts it). Hide the card until a hosted page exists; share the PDF — #80
- [ ] **AUDIT P1 batch** — verdict band/recommendation shown with zero evidence (gate on analyzed photos); "Generate PDF & share" demotes signed → sent (never overwrite signed; mark sent after the share resolves); one accidental stroke signs (confirm step + min stroke length); §3 collateral checkboxes → `structural.layers` / `substrate_exposure` / brittleness in `engineInputFromInspection`; inspector-added markers carry evidence + `deriveFunctional` inside `withRecount`; Analyze "Next — go to the job" becomes the 88pt primary when nothing is left; search→lead opens `/lead/[id]`; door-knock leads reverse-geocoded + lead edit sheet; estimator `regionForState(parseState(address))`; onboarding "10 hits" → engine constant; "hail ≥0.75"" copy vs the 0.25" floor; proposal auto-create gated on `persist.hasHydrated()` — #80
- [ ] **AUDIT P2 batch** — safety rating printed without a forecast; Convert-to-inspection flips the stage before the wizard saves; Leads empty state → /new-job; Plan "schedule" is createdAt; mileage rate hard-coded 0.67; delete trip / clear activity / delete voice note without a confirm sheet; follow-up push tap unhandled; map lead pins have no callout; developer copy ("Add EXPO_PUBLIC_GEMINI_API_KEY to .env.local") shown to the roofer; ~34pt header back buttons on 11 screens; 33 inline hex literals across 9 route files (Drift #11); partial material pickers; estimator Done discards silently; Pipeline value never renders; /settings/diagnostics hydration mismatch on web — #80
...
- [ ] **Knock-finder follow-ups** — (a) calibrate §4.1 base rates from found-vs-expected per stop (log it from the knock session's damage-confirmed outcomes — learning loop pattern); (b) owner: get the free Census key (api.census.gov/data/key_signup.html) into `.env.local` so roof age scores; (c) draw the ranked areas as rings on Storm Tracer when arriving from the finder (today it lands on the cell centre); (d) per-stop door count as a setting (40 today) — #81
