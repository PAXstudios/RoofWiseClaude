...
- [ ] **Knock-finder follow-ups** — (a) calibrate §4.1 base rates from found-vs-expected per stop (log it from the knock session's damage-confirmed outcomes — learning loop pattern); (b) owner: get the free Census key (api.census.gov/data/key_signup.html) into `.env.local` so roof age scores; (c) per-stop door count as a setting (40 today); (d) draw ALL ranked areas as rings at once (one ring per visit today, #83) — #81
