...
- [ ] **Knock Planner next (owner asked for ideas, #93)** — (a) weather-aware day pick: the forecast (`weatherForecast.ts`) chooses the dry day and the 4–7 pm weekday / 10–2 weekend windows answer rates favour; (b) auto-plan on a damaging storm alert: Storm Watch queues a plan for the alert's core and the bell says "plan ready for the Jun 14 storm"; (c) plan performance: doors / contacts / leads / signed per area from the sessions inside its ring → the base-rate calibration the formula is waiting for (docs/KNOCK_OPPORTUNITIES.md §8); (d) all ranked areas as rings on Storm Tracer at once; (e) share / export the plan (PDF + share sheet) and assign areas to crew (needs Team roles); (f) do-not-knock zones (HOA no-solicit lists) excluded from ranking; (g) "for-sale homes here" beside recently-sold and a listing-agent list per plan; (h) the plan's days on the Plan tab as scheduled stops with start times; (i) neighbours-of-my-jobs mode ("lead with the yard sign on Oak St"); (j) after Wave E lands: "Start this day" → `setRouteStops` with every stop — #93
...
- [ ] **Multi-stop knock routes** — `KnockSession.routeStops[]` so a trip plan's stops ride the session in order (single `routeTarget` today) — #80
