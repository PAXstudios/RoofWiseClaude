/**
 * Seeded simplex noise (2D + 3D) with a fractal Brownian motion helper.
 * Written from scratch following Stefan Gustavson's public-domain reference
 * implementation, so the engine has zero noise-library dependencies.
 */
...
/** Mulberry32: tiny deterministic PRNG used to shuffle the permutation table. */
...
export class SimplexNoise {
  ...
  noise2D(xin, yin) {
    ...
    // Which triangle of the simplex cell are we in?
    const i1 = x0 > y0 ? 1 : 0;
    const j1 = 1 - i1;
  ...
  noise3D(xin, yin, zin) {
    ...
    // Rank the coordinates to find which simplex (tetrahedron) we are in.
  ...
  /** Fractal Brownian motion over noise2D, normalized to roughly [-1, 1]. */
