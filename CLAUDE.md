...
| Knock Planner runs, saved plans, notifications | `lib/services/knockPlanRunner.ts` (the background run — never run the finder from a screen), `lib/stores/knockFinderStore.ts` (`plans` by date, `activeRun`, area statuses), `components/knock/PlanView.tsx` + `app/knock-plan/[id].tsx` (the interactive plan page), `lib/stores/notificationStore.ts` + `app/notifications.tsx` (the bell; push entries with a `key` so "working" becomes "ready" in place) |
